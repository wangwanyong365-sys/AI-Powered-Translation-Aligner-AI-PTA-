...
class APIProviderDialog(tk.Toplevel):
    ...
    def _create_widgets(self, parent):
        ...
        self.type_combo = ttk.Combobox(form_frame, textvariable=self.provider_type_var, state="readonly", values=["Standard", "Azure (with API Version)", "Azure (without API Version)"])
    ...
    def _on_type_change(self, event=None):
        ...
        if ptype == "Standard":
            ...
            if ptype == "Azure (with API Version)":
                self.api_version_label.grid()
                self.api_version_entry.grid()
            else: # Azure (without API Version)
        ...
        if self.is_editing:
            ...
            if "base_url" in self.provider_data:
                ...
                if "api_version" in self.provider_data:
                    self.provider_type_var.set("Azure (with API Version)")
                    self.api_version_var.set(self.provider_data.get("api_version", ""))
                else:
                    self.provider_type_var.set("Azure (without API Version)")
...
class SettingsDialog(tk.Toplevel):
    ...
    def _create_api_performance_settings(self, parent_frame):
        ...
        self.request_interval = tk.IntVar(value=self.parent.settings.get("request_interval", 0))
